...
const WorkflowBuilder = () => {
  const [nodes, setNodes] = useState([
    { id: 1, type: 'trigger', icon: Bot, label: 'AI Trigger' }
  ]);
  ...
  const addNode = (nodeType: { type: string; icon: React.ForwardRefExoticComponent<Omit<LucideProps, "ref"> & React.RefAttributes<SVGSVGElement>>; label: string; id?: number; }) => {
    setNodes(prev => [...prev, {
      id: prev.length + 1,
      ...nodeType
    }]);
  ...
  return (
    ...
    <div className="w-full max-w-4xl mx-auto p-6">
      ...
      <div className="grid grid-cols-2  md:grid-cols-4 gap-4">
        {nodeTypes.map((type) => (
          <Card key={type.label} className="hover:shadow-lg transition-shadow duration-200 cursor-pointer">
