...
import { 
  MessageSquare, Mail, Phone, Slack, 
  Bot, FileSpreadsheet, FileText, Calendar, Video,
  MessageCircle, Briefcase, Building2, Database
} from "lucide-react";
...
export default function IntegrationBeams() {
  ...
  return (
    ...
    >
      {/* Center AI Hub */}
      <Circle ref={centerRef} className="absolute left-1/2 top-1/2 size-16 -translate-x-1/2 -translate-y-1/2 bg-primary">
      ...
      {apps.map((app, index) => {
        ...
        return (
          <div key={app.label} className="absolute" style={{
            left: `calc(50% + ${x}px)`,
            top: `calc(50% + ${y}px)`,
          }}>
            <Circle
              ref={el => appRefs.current[index] = el ?? null}
